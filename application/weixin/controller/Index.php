...
class Index extends Common
{
	...
	public function shareWx(){
		...
		// 获取signature
		$signature = $this->getSingature($url,$time,$noncestr,$jsapi_ticket);
		$this->assign('name','李超');
	...
	// 获取signature
	public function getSingature($url,$time,$noncestr,$jsapi_ticket){
	...
	public function getTimeQrCode(){
		// 1.获取ticket
		// 全局票据包括access_token 网页授权accexx_token 微信js-sdk jsapi_ticket 二维码 ticket
		$access_token = $this->getWxAccessToken();
		$url = 'https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token='.$access_token.'';
		$postArr = array(
			'expire_seconds' => 604800, //24*60*60*7
			'action_name' =>  'QR_SCENE', // 二维码类型QR_SCENE为临时的整型参数值，QR_STR_SCENE为临时的字符串参数值，QR_LIMIT_SCENE为永久的整型参数值，QR_LIMIT_STR_SCENE为永久的字符串参数值
			'action_info' => array(
				'scene' => array(
					'scene_id' => 2000
				),
	...
	public function getForverQrCode(){
		...
		$postArr = array(
			'expire_seconds' => 604800, //24*60*60*7
			'action_name' =>  'QR_LIMIT_SCENE', // 二维码类型QR_SCENE为临时的整型参数值，QR_STR_SCENE为临时的字符串参数值，QR_LIMIT_SCENE为永久的整型参数值，QR_LIMIT_STR_SCENE为永久的字符串参数值
			'action_info' => array(
				'scene' => array(
					'scene_id' => 2000
				),
